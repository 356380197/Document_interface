...
$(document).ready(function(e) {
	...
	var suc=true;
	var isloading = false;
	var prize_url=http_url+"/activity/activity/view";
	var list_url=http_url+"/activity/posts/list";
...
//				    myrefresh();
		...
		appendactlist:function(datalist,isRanking){
			...
			for(var i=0;i<datalist.results.length;i++){
				...
				prize_list='<div class="dynamic_box" id="'+datalist.results[i].id+'">'
				          ...
				          +'</div>'
				          +'<div class="clear"></div>'
				          +'<div class="dyn_articl" i="'+datalist.results[i].id+'">'+datalist.results[i].content+'</div>'
				          +'</div>'
		...
		appendactpic:function(datalist,tietid,i){
			...
			if(datalist.results[i].pics.length>0){
				...
				$("#"+tietid).find(".allcomcz").before(photobox);
...
//					          +'<img src="'+datalist.results[i].pics[x][0]+'" width="100%" style="display:none;">'
					          +'</a>'
					          +'</li>';
	...
	$(window).scroll(function () {
		if(suc==true){
			...
			if (scrollTop + windowHeight == scrollHeight) {
				if ($(".tab_show").length > 0 && isloading == false ) {
					isloading = true;
