...
$(document).ready(function(e) {
    var hotcom_url=game_http_url+"/activity/posts/view";
	var morecom_url=game_http_url+"/activity/comment/list";
	var send_url=game_http_url+"/activity/comment/save";
	var zan_url=game_http_url+"/activity/praise/save";
	var deltie_url=game_http_url+"/activity/posts/remove";
	var delcom_url=game_http_url+"/activity/comment/remove";
	...
	var fasong=document.getElementById('sendcom');
//	alert(1);
	var allcom_start={
		loading:$(".loading"),
		mack:$(".mack"),
		...
		ajax1:function(url,datacan,activityId){
			$.ajax({
				url:url,
				contentType:contenttype,
				type:"POST",
				dataType:"JSON",
				data:JSON.stringify(datacan,activityId),
				timeout:10000,
		...
		ajax2:function(url,userId,postsId,content,parentId,activityId){
			$.ajax({
				url:url,
				contentType:contenttype,
				type:"POST",
...
//					}
		...
		appendnewcom:function(data){
			...
			var copycom='<div class="person fl">'
			            ...
			            +'<span class="zan tiezan"><a praiseid="'+data.data.praiseId+'"><img src="images/icon/icon4.png" width="13" height="12"><i class="zannum">'+data.data.praiseNum+'</i></a></span>'
		...
		appendtie:function(datalist,url){
			...
			var tie='<div class="dynamic_box tiebox">'
			     +'<div class="gamebox">'
			     +'<div class="gametit">'
			     +'<span>第'+datalist.matchRanking.rank+'名</span>'
			     +'<span>'+datalist.matchRanking.childName+'</span>'
			     +'<span>'+datalist.matchRanking.voteNum+'票</span>'
			     +'</div>'
			     +'</div>'
			     +'<div class="person fl">'
			     +'<img src="'+peravatar+'" width="100%"/>'
			     ...
			     +'<div class="dyn_cz allcomcz">'
			     +'<span class="zan tiezan"><a praiseid="'+datalist.praiseId+'"><img src="'+zansrc+'" width="13" height="12"><i tartype="0" class="zan_com">'+comnum+'</i></a></span>'
...
//					         +'<a href="'+datalist.pics[i][0]+'"><img src="'+datalist.pics[i][1]+'" width="100%"></a>'
		...
		appendmorecom:function(datalist,url,pageNo,activityId){
		...
		addcom:function(datalist,url,isonline,status){
			...
			for(var n=0; n<datalist.length;n++){
				...
				var hotcom='<div class="person fl">'
				           ...
				           +'<div class="dyn_cz allcomcz">'
				           +'<span class="zan tiezan"><a praiseid="'+datalist[n].praiseId+'"><img src="'+zansrc+'" width="13" height="12"><i class="zan_num">'+comnum+'</i></a></span>'
		...
		init:function(){
			...
			$("body").on("touchend","#sendcom",function(){
				...
				if(content!=""&&content!="回复楼主"&&content!="回复"+anstext){
					allcom_start.ajax2(send_url,userId,id,content,parentId,activityId);
